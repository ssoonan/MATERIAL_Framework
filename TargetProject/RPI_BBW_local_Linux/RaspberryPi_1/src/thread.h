/*
 * thread.h - Linux version using POSIX threads
 */

#ifndef THREAD_H_
#define THREAD_H_

#define _GNU_SOURCE  // Enable CPU affinity functions
 ...
 */
struct thread_data {
    // thread id as assigned by pthread
    ...
    // core affinity of this thread (CPU mask)
    cpu_set_t cpu_mask;
    ...
    // No APS - simplified for minimal approach
    int partition_id;  // Keep for compatibility but not used
...
struct thread_data* thread_addThreadDescription(
        ...
        int _partition_id);  // Keep for compatibility
...
int thread_setThreadParameters(int policy, int priority, pthread_t tid);
...
struct thread_data* thread_getThreadFromTid(pid_t tid);
